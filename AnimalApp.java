	  ...
	  Calendar now = Calendar.getInstance();   
		...
		for(int i=0; i<SIZE; i+=1){
			if(Math.random() < 0.5){
				...
				animals[i] = new Owl( names[rnd.nextInt(names.length)], rnd.nextInt(16) + 2000, rnd.nextBoolean());
			}else {
				animals[i] = new Jaguar( names[rnd.nextInt(names.length)], rnd.nextInt(18) + 2000);
			}
		}
